...
openWindowEx title position size (RedrawMode useDoubleBuffer) = do
  ...
  let motionCallback (GL.Position x y) =
        atomically $ writeTChan eventsChan MouseMove { pt = (fromIntegral x, fromIntegral y) }
...
maybeGetWindowEvent win = do
  ...
  if noEvents 
    then GLFW.sleep 0.01 >> GLFW.pollEvents >> return Nothing
      ...
      event <- atomically $ readTChan (eventsChan win)
      case event of
        Refresh -> do
	  ...
	  atomically $ writeTChan (eventsChan win) Refresh
          maybeGetWindowEvent win
        e -> return (Just e)

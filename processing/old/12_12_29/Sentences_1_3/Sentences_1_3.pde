...
void loadVars() {
  String[] tmpS = loadStrings("vars.txt");
  if(tmpS != null && tmpS.length>2) {
    tOffsetX = float(tmpS[0]);
    tSize = float(tmpS[1]);
    interval = int(tmpS[2]);
  }
}
...
void doKeyControls() {
  if(keyPressed) {
    ...
    if(key == 'd') tOffsetX ++;
    // text time control
    if(key == 'w') interval += 10;
    if(key == 's') interval -= 10;
    // text size control
    if(key == 'q') tSize -= 0.2;
    if(key == 'e') tSize += 0.2;
...
void saveVars(){
  String[] tmpS = { str(tOffsetX), str(tSize), str(interval) };
  saveStrings("data/vars.txt", tmpS);
}
...
void showVarInfo(){
  ...
  textAlign(LEFT);
  textFont(fonts[0], 15);
  text("Interval: " + interval, width*0.5, height*0.8);
  text("Space: " + int(tOffsetX), width*0.5, height*0.84);
  text("Size: " + int(tSize), width*0.5, height*0.88);
}
...
void renderSentences() {
  fill(255);
  textFont(fonts[fontNum], tSize);
  float y = height*0.5;
  float x = tOffsetX;
  textAlign(CENTER);
  text(ss[ID1].t, width*0.5-x, y);
  text(ss[ID2].t, width*0.5+x, y);
}

class Player{
  ...
  Player(Vect2 posision, String controls, String id){
    ...
    weaponList[5] = "100_forcewave";
    weaponList[6] = "100_blackhole";
    currentWeapon = 5;
  }
  ...
  void hitMissiles(){
    float hitDistance = 0; //extend*0.5;
    for(int i=0; i<missiles.size(); i++){
      hitDistance = ((Missile) missiles.get(i)).vol + extend;
  ...
  void hitAstroids(){
    for(int i=0; i<astroids.size(); i++){
      float tmpVol = ((Astroid) astroids.get(i)).vol;
      Vect2 tmpPos = new Vect2(((Astroid) astroids.get(i)).pos);
      Vect2 tmpDelta = new Vect2(pos);
      tmpDelta.sub(tmpPos);
      float tmpMinDist = (extend*0.5) + tmpVol;

      if(tmpDelta.x*tmpDelta.x + tmpDelta.y*tmpDelta.y < tmpMinDist*tmpMinDist){
        // set new posision
        float tmpAngle = atan2(tmpDelta.y, tmpDelta.x);
        Vect2 angleVector = new Vect2(cos(tmpAngle), sin(tmpAngle));
        Vect2 newPos = new Vect2(angleVector.x, angleVector.y);
        newPos.mult(tmpMinDist);
        newPos.add(tmpPos);
        pos.set(newPos);

        // set new acceleration
        Vect2 tmpVel = new Vect2(((Astroid) astroids.get(i)).vel);
        Vect2 newAcc = new Vect2(angleVector.x, angleVector.y);
        newAcc.mult(tmpVel.magnitude());
        acc.add(newAcc);
        
        // damage
        Vect2 tmpVelSum = new Vect2(vel);
        tmpVelSum.sub(tmpVel);
        float tmpDmg = tmpVelSum.magnitude();
        life -= tmpDmg;
        
        int tmpTextSize = int(tmpDmg*textSizeDmgFactor + textSizeMin);
        textPops.add(new TextPop(str(int(tmpDmg)), pos, tmpTextSize, 15, 255, "001100"));
      }
    }
  }
  ...
  void shoot(){
    if(cooldownTimer < 1){
      ...
      Vect2 tmpCannonPos = new Vect2(pos.x+cos(direction)*extend*3, pos.y+sin(direction)*extend*3);
  ...
  void render(){
    fill(skin);
    noStroke();
    ellipse(pos.x,pos.y,extend*2,extend*2);
    stroke(155);
    strokeWeight(3);
    float cannonLength = 2/(cooldownTimer*0.1+1);

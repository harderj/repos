// Harder2DLib
// Builds on the library point2line by Carl Emil Carlsen, www.sixthsensor.com
// Contains classes:
// - Point2 (float x, float y)
// - Line2 (Point2 p1, Point2 p2)
// - Circle2 (Point2 p, float r)
// - Poly2 (Point2[] p)
// Also contain a few loose and supporting functions such as:
// - intersectingMultipleLines(Line2[] l)
// - negative(color c)
...
boolean insideInterval(float val, float min, float max){
  if(val >= min(min,max) && val <= max(min,max)) return true;
  else return false;
}
